...
import { usePickupOrder, useInTransitOrder, useCompleteOrder } from '../../../entities/order/api/courierApi';
...
export const ActiveOrderCard = ({ order }: ActiveOrderCardProps) => {
  ...
  const getButtonText = () => {
    switch (order.status) {
      case OrderStatus.COURIER_ASSIGNED: return 'I am here / Picked up';
      case OrderStatus.PICKED_UP: return 'Start Delivery';
      case OrderStatus.DELIVERING: return 'Complete Delivery';
      default: return 'Next Step';
    }
  };

  const isPending = pickupMutation.isPending || transitMutation.isPending || completeMutation.isPending;
    ...
    <div className="bg-white border-2 border-blue-500 rounded-xl p-6 shadow-lg mb-8 relative">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="flex justify-between text-sm">
          <span>Cargo: <span className="font-bold">{order.cargoType}</span></span>
          <span>Price: <span className="font-bold text-green-600">${order.price.toFixed(2)}</span></span>
        </div>
      </div>
